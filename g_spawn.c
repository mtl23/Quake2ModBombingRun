...
void ReplaceItem(edict_t *ent, gitem_t *item);
...
void ReplaceItem(edict_t *ent, gitem_t *item)
{
	...
	char *sp_item[1][2]=
		{
			{"item_quad","weapon_griffball"}, //to replaces quad damage with griffball(s)
		};
			
			if(Q_stricmp(ent->classname,sp_item[0][0])==0)
			{			
					ent->classname = item->classname = sp_item[0][1];
					SpawnItem(ent,item);
					//VectorSet(ent->s.origin,-220,180,200);//set to the center of the stage
