...
void CTFInit(void)
{
	...
	if (!flag1_item)
		flag1_item = FindItemByClassname("weapon_griffball");
	if (!flag2_item)
		flag2_item = FindItemByClassname("weapon_griffball");
...
void CTFFragBonuses(edict_t *targ, edict_t *inflictor, edict_t *attacker)
{
	...
	if (targ->client->pers.inventory[ITEM_INDEX(enemy_flag_item)]) {
		...
		gi.cprintf(attacker, PRINT_MEDIUM, "BONUS: %d points for fragging enemy ball carrier.\n",
	...
	if (targ->client->resp.ctf_lasthurtcarrier &&
		...
		gi.bprintf(PRINT_MEDIUM, "%s defends %s's ball carrier against an agressive enemy\n",
	...
	if (carrier && carrier != attacker) {
		...
		if (VectorLength(v1) < CTF_ATTACKER_PROTECT_RADIUS ||
			...
			gi.bprintf(PRINT_MEDIUM, "%s defends the %s's ball carrier.\n",
				attacker->client->pers.netname, 
				CTFTeamName(attacker->client->resp.ctf_team));
			return;
		}
...
qboolean CTFPickup_Flag(edict_t *ent, edict_t *other)
{
	...
	if (ctf_team != other->client->resp.ctf_team) {
		...
		if (!(ent->spawnflags & DROPPED_ITEM)) {
			...
			if (other->client->pers.inventory[ITEM_INDEX(enemy_flag_item)]) {
				gi.bprintf(PRINT_HIGH, "%s scored for the %s team!\n",
				...
				if (ctf_team == CTF_TEAM1)
					...
					ctfgame.team1++;
				ball->classname="weapon_griffball";
				
				SpawnItem (ball, flag1_item);// mtl should spawn a grffball see g_spawn 327
	...
	//gi.bprintf(PRINT_HIGH, "%s got the %s flag FIXME!\n",
		//other->client->pers.netname, CTFTeamName(ctf_team));
...
void CTFWeapon_Grapple (edict_t *ent)
{
	...
	Weapon_Generic (ent, 5, 9, 31, 36, pause_frames, fire_frames, 
		CTFWeapon_Grapple_Fire,NULL,NULL,false);
